...
<body>
    ...
    echo "Please finish you registration by setting up your profile."."<br>"."<br>";
    ...
    if( !empty($_SESSION["username"]) )
    {
        ...
        $peopleID = mysql_query( "SELECT staffID from registered_staff WHERE username='$username'");
        $peopleID = mysql_fetch_row($peopleID);
        $peopleID = $peopleID[0];
    }
    ...
    if( $_SERVER["REQUEST_METHOD"] == "POST" )
    {
        ...
        {
        $folder   = "uploads/";
        $pic      = $_FILES['headshot']['name'];
        $pic_loc  = $_FILES['headshot']['tmp_name'];
        ...
        echo $target_dir;    
            ...
            //storePersonalInfo( $peopleID, $fname, $middleI, $lname, $nickname, $address, $phone);
            $query = "UPDATE registered_staff
                      SET firstName='$fname',lastName='$lname',gender='$gender',nativeLanguage='$languages',phone='$phone',address='$address'
                      WHERE staffID='$peopleID'";
            $res = mysql_query($query,$db) ;
            if($res) echo "yes";else echo "no";
            //header("Location: http://45.55.240.59/Website/registration_success.php");
        }
